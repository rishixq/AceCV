	...
	return (
		<form onSubmit={handleSubmit(handleFormSubmit)}>
			...
			{fields.map((field, i) => {
				return (
					<div
						className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-4 mt-8 font-bold"
						key={field.id}
					>
						<Input
							label="Skill"
							{...register(`skills.${i}.skill`, { required: true })}
						/>
						<Input
							label="Rating"
			...
			<div className="flex justify-between items-center max-w-4xl mt-40">
				...
				<button
					type="submit"
					className="bg-indigo-700  text-white py-3 px-10 rounded-lg mt-8 hover:scale-105 transition-all ease-in-out"
				>
					Next{" "}
				</button>
			</div>
